...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const [termo, cfg] = await Promise.all([buscarTermoPorSlug(params.slug), getConfig()])
  if (!termo) return { title: 'Não encontrado' }
  return {
    title:       termo.seoTitle || `${termo.termo} — ${cfg.siteName}`,
    description: termo.resumo   || `Saiba o que é ${termo.termo} no glossário de ${cfg.siteNiche}.`,
    alternates:  { canonical: `${cfg.siteUrl}/glossario/${termo.slug}` },
    openGraph: {
      title:       termo.seoTitle || termo.termo,
      description: termo.resumo || '',
      url:         `${cfg.siteUrl}/glossario/${termo.slug}`,
    },
  }
}
